#!/usr/bin/env python3
"""
TTS Groq Bridge - WebSocket to HTTP Bridge for Text-to-Speech
...
Based on the successful STT bridge pattern but reversed data flow:
...
logger.info(f"TTS Bridge starting with model: {GROQ_TTS_MODEL}, voice: {GROQ_TTS_VOICE}")


class TTSBridge:
    """Text-to-Speech WebSocket bridge for Groq API integration."""
    ...
    async def handle_websocket_connection(self, websocket: WebSocketServerProtocol, path: str):
        """
        Handle WebSocket connection from Unmute service.
        ...
        try:
            ...
            logger.info(f"TTS WebSocket connection #{connection_id} ended")
...
async def main():
    ...
    try:
        logger.info(f"Starting TTS Groq Bridge on port {WS_PORT}")
        ...
        logger.info("TTS Bridge server started successfully")
        ...
        logger.info("TTS Bridge server stopped")
